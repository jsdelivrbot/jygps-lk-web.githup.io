  ...
  <div class="content-container" style="padding:0 10px;margin-top:10px;">
    <section>
      ...
      <el-table class="row" id="pdfDom" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
        element-loading-background="rgba(0, 0, 0, 0.8)" :data="allnum" width="100%" :height="windowHeigh"
        highlight-current-row style="border-right:solid 1px #dfe6ec" @selection-change="selsChange">
        <!-- 选择的按钮 多选按钮 -->
        ...
        <el-table-column label="报警状态" min-width="100" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <!-- 下面2行代码，因为利用level值判断不准确，要用温湿度的值来判断报警状态，
            判断温湿度是否存在，不存在赋值给0，再判断温湿度是否再最大值最小值之间，超出报警，否则正常 -->
            
            <span v-if="(
            ((scope.row.tmp1Current==undefined)&&(scope.row.humidity==undefined))||
            ((scope.row.tmp1Current==undefined)&&
            (scope.row.humidity)-(scope.row.hum1min)>=0&&(scope.row.humidity)-(scope.row.hum1max)<=0)||
            ((scope.row.humidity==undefined)&&
            (scope.row.tmp1Current)-(scope.row.tmp1min)>=0&&(scope.row.tmp1Current)-(scope.row.tmp1max)<=0)||
            (scope.row.hum1max==undefined&&scope.row.hum1min==undefined&&scope.row.tmp1max==undefined&&scope.row.tmp1min==undefined)||
            (scope.row.hum1max==undefined&&scope.row.hum1min==undefined&&scope.row.tmp1min==undefined&&scope.row.tmp1Current<scope.row.tmp1max)||
            (scope.row.hum1max==undefined&&scope.row.hum1min==undefined&&scope.row.tmp1max==undefined&&scope.row.tmp1Current>scope.row.tmp1min)||
            (scope.row.tmp1max==undefined&&scope.row.tmp1min==undefined&&scope.row.hum1max==undefined&&scope.row.humidity>scope.row.hum1min)||
            (scope.row.tmp1max==undefined&&scope.row.tmp1min==undefined&&scope.row.hum1min==undefined&&scope.row.humidity<scope.row.hum1max)||
            ((scope.row.humidity)-(scope.row.hum1min)>=0 && (scope.row.humidity)-(scope.row.hum1max)<=0)&&
            ((scope.row.tmp1Current)-(scope.row.tmp1min)>=0&&(scope.row.tmp1Current)-(scope.row.tmp1max)<=0)||
            (scope.row.hum1max==undefined&&scope.row.hum1min==undefined&&
            (scope.row.tmp1Current)-(scope.row.tmp1min)>=0&&(scope.row.tmp1Current)-(scope.row.tmp1max)<=0)||
            (scope.row.tmp1max==undefined&&scope.row.tmp1min==undefined&&
            (scope.row.humidity)-(scope.row.hum1min)>=0&&(scope.row.humidity)-(scope.row.hum1max)<=0)
            )
            ">设备正常</span>
